...
export default function AddUserModal({ open, setOpen }) {
    ...
    //     dispatch(createUser({username:userData.name,password:userData.password ,phone:userData.phone})).then(res => {
    //         console.log(res)
    ...
    return (
        ...
        >
            <form className="flex flex-col gap-3">
                ...
                <button className="bg-[#0d6efd] hover:bg-[#104ba3]  text-white p-2 rounded-sm w-full">{addLoading ? t("loadingText") : t("saveBtn")}</button>
            </form>
        </Modal>
    );
}
